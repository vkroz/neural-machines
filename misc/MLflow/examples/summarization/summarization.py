...
def build_and_evaluate_model_with_prompt(prompt_template):
    with mlflow.start_run():
        ...
        # Create a news summarization model using prompt engineering with LangChain
        llm = ChatOpenAI(temperature=0.9)
        prompt = PromptTemplate(input_variables=["article"], template=prompt_template)
        chain = LLMChain(llm=llm, prompt=prompt)

        # Log the model to MLflow Tracking
        signature = infer_signature({"article": "sample text"}, {"text": "sample summary"})
        mlflow.langchain.log_model(chain, artifact_path="model", signature=signature)
...
# Run experiments with both prompts
...
results = mlflow.load_table("eval_results_table.json", extra_columns=["run_id", "params.prompt_template"])
